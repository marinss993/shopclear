<?php
    /*
    * @version      1.4.2 19.07.2018
    * @author       MAXXmarketing GmbH
    * @package      addon_corext.php
    * @copyright    Copyright (C) 2010 webdesigner-profi.de. All rights reserved.
    * @license      GNU/GPL
    */
    defined('_JEXEC') or die;

    abstract class AddonApiAddonCorext extends AddonApiSingleton {
        ...
        public function __construct() {
            ...
            $this->table->loadAlias($this->getAlias());
            $this->id = $this->getTable()->id;
            $this->setParams();
            ...
            JSFactory::loadExtLanguageFile($this->getAlias());
        ...
        protected function setParams() {
            ...
            foreach (JFolder::folders(JPATH_JOOMSHOPPING_ADMIN) as $dir) {
                switch ($dir) {
                    ...
                    case 'models':
                        ...
                        break;
                    default:
                        $this->params['dirs']['admin_' . $dir] = 'administrator/components/com_jshopping/' . $dir . '/' . $this->getAlias();
                }
            }
            foreach (JFolder::folders(JPATH_JOOMSHOPPING) as $dir) {
                switch ($dir) {
                    ...
                    case 'js':
                    case 'templates':
                        $this->params['dirs'][$dir] = 'components/com_jshopping/' . $dir . '/addons/' . $this->getAlias();
                        break;
                    default:
                        $this->params['dirs'][$dir] = 'components/com_jshopping/' . $dir . '/' . $this->getAlias();
                }
            }
            foreach (JFolder::folders(JPATH_JOOMSHOPPING . '/addons/' . $this->getAlias()) as $dir) {
            ...
            if (empty($this->params['dirs']['tmp'])) {
                $this->params['dirs']['tmp'] = str_replace(
                    ...
                    '',
                    JPath::clean(
                        JFactory::getConfig()->get('tmp_path', JPATH_ROOT . '/tmp') . '/' . $this->getAlias()
                    )
                );
            }
            /* treat specials */
        ...
        public function sessionSet($name, $value) {
            return JFactory::getSession()->set($name, $value, $this->getAlias());
        }

        public function sessionGet($name, $default = null) {
            return JFactory::getSession()->get($name, $default, $this->getAlias());
        }

        public function sessionClear($name) {
            return JFactory::getSession()->clear($name, $this->getAlias());
        }
        ...
        public function renderPlgView($layout, $vars = [], $type = '') {
            ...
            if (!$type) {
                ...
                $type = preg_replace('/' . $this->getAlias() . '$/i', '', $type);
            }
            ob_start();
            include JPluginHelper::getLayoutPath($type, $this->getAlias(), $layout);
            return ob_get_clean();
        }
        ...
        protected function addJsDefaultVars() {
            ...
            $prefix = '_JSHOP_' . strtoupper($this->getAlias()) . '_';
            foreach ([
                'OK'
            ] as $name) {
        ...
        public function addJsVar($name, $val) {
            JFactory::getDocument()->addScriptOptions($this->getAlias(), [$name => $val]);
            return true;
        }
        ...
        public function getModel($name = '') {
            return JSFactory::getModel(
                strtolower(
                    trim(
                        $name ? ($this->getAlias() . '_' . $name) : $this->getAlias()
                    )
                )
            );
        }

        public function getModule($alias = '', $published_only = false) {
        ...
        public function log($msg, $file = '', $line = 0, $code = 0, array $extras = [], $filepath = null) {
            ...
            if ($filepath === null) {
                $filepath = JPath::clean(
                    JSFactory::getConfig()->log_path . '/' . $this->getAlias() . '.log.ini'
                );
            }
            /* collect parts */
            $parts = [
        ...
        public function getLangsef($current = true) {
            static $langs;
            $tag = strtolower(
                trim(
                    $current
            ...
            foreach ($langs ? $langs : $langs = getAllLanguages() as $lang) {
                if (strtolower(trim($lang->language)) == $tag) {
                    return $lang->lang;
                }
            }
            return 'en';
        ...
        public function getLogPath() {
            return JPath::clean(JSFactory::getConfig()->log_path . $this->getAlias() . '.log.ini');
        }
